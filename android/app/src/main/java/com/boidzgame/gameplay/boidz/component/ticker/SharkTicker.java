...
public class SharkTicker extends TickerComponent {
     ...
     */
    private static final double hungerF = 100.0d;
    /**
    ...
    private static final int oscillationPeriod = 20;
    private static final double oscillationAmplitude = 2.0d;
    ...
    public void tick(double delay) {
        ...
        if (closest != null) {
            ...
            } else if(mLevel.touchManager.fingersList.size() == 0) {
                // attraction to the closest
                accNorm = Math.sqrt(closestDist.x * closestDist.x + closestDist.y * closestDist.y);
                acc.x += closestDist.x * hungerF / accNorm;
                acc.y += closestDist.y * hungerF / accNorm;
            }
        }

        oscillationCount++;
        oscillationCount %= oscillationPeriod;
        oscillation = Math.sin(2 * Math.PI * oscillationCount / oscillationPeriod)
